...
class ConvDeconvWithSkipConnection(nn.Module) :
    ...
    def forward(self,x):
        ...
        x42_dec = F.relu(self.decoder_layer42_batch(self.decoder_layer42_conv(x50_dec+x42)))
        ...
        x32_dec = F.relu(self.decoder_layer32_batch(self.decoder_layer32_conv(x40_dec+x32)))
        ...
        x21_dec = F.relu(self.decoder_layer21_batch(self.decoder_layer21_conv(x30_dec+x21)))
        x20_dec = F.relu(self.decoder_layer20_batch(self.decoder_layer20_conv(x21_dec)))

        x11_dec = F.relu(self.decoder_layer11_batch(self.decoder_layer11_conv(x20_dec+x11)))
